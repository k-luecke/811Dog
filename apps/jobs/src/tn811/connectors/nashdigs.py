"""
connectors/nashdigs.py — pull Northstar-owned fiber projects from NashDigs.
...
with ProjectName = the BNA### package identifier Northstar (and Meridian) already
use internally. This connector:

  1. Queries the public FeatureServer for all `Owner = 'Northstar'` line features
  ...
  3. Upserts into the local ORMNashDigsProject table (keyed on ProjectID),
     ...
     were cancelled or left Northstar ownership).
...
# Owner filter — safety check confirmed Owner='Northstar' == Department='Northstar'
...
class NashDigsPackage:
    """A single Northstar-owned fiber project pulled from NashDigs."""
...
def fetch_target_packages(
    client: httpx.Client | None = None,
    where: str = DEFAULT_WHERE,
) -> list[NashDigsPackage]:
    """Query NashDigs Layer 1 (Lines) for Northstar-owned packages.
