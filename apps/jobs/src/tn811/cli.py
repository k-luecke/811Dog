...
@click.option(
    "--all-owners",
    is_flag=True,
    help="Also pull non-Google projects (Lines + Polygons) for the KMZ conflict layer.",
)
def fetch_nashdigs(config_path: str, dry_run: bool, all_owners: bool):
    ...
    With --all-owners, also pulls Lines + Polygons from every non-Google owner
    (Metro Water, NES, NDOT, etc.) to populate the KMZ conflict-awareness
    layer. Stale-deletion is scoped by owner, so a subsequent Google-only
    refresh won't wipe the non-Google cache.
    """
    cfg = _load_app(config_path)

    from tn811.connectors.nashdigs import (
        fetch_google_packages,
        fetch_non_google_projects,
        upsert_packages,
    )
    ...
    if all_owners:
        others = fetch_non_google_projects()
        click.echo(f"NashDigs: fetched {len(others)} non-Google feature(s) for conflict layer.")
