...
from tn811.pdf.extractor import ExtractedFields
...
def _make_extracted(**kwargs) -> ExtractedFields:
    defaults = dict(
        ticket_number="TN20240601-100001",
        county="Davidson County",
        ...
        work_type="Telecom - Underground Fiber",
        location_text="123 Main St Nashville TN",
        remarks="Northstar Fiber drop bury FTTH",
    )
    defaults.update(kwargs)
...
class TestNormalizeTicket:
    def test_returns_normalized_ticket(self):
        row = _make_row()
        t = normalize_ticket(row, None, None, now=NOW)
    ...
    def test_pdf_data_preferred_over_detail(self):
        row = _make_row()
        detail = _make_detail(fields={"excavator_name": "Detail Corp"})
        extracted = _make_extracted(excavator_name="PDF Corp")
        t = normalize_ticket(row, detail, extracted, now=NOW)
        assert t.excavator_name == "PDF Corp"

    def test_detail_data_preferred_over_row(self):
        row = _make_row(excavator_name_raw="Row Corp")
        detail = _make_detail(fields={"excavator_name": "Detail Corp"})
        t = normalize_ticket(row, detail, None, now=NOW)
    ...
    def test_expiration_date_parsed_from_extracted(self):
        row = _make_row()
        extracted = _make_extracted(expiration_date=date(2024, 6, 16))
        t = normalize_ticket(row, None, extracted, now=NOW)
    ...
    def test_active_status_derived(self):
        row = _make_row()
        extracted = _make_extracted(expiration_date=date(2099, 12, 31))
        t = normalize_ticket(row, None, extracted, now=NOW)
        assert t.status == TicketStatus.ACTIVE

    def test_expired_status_derived(self):
        row = _make_row()
        extracted = _make_extracted(expiration_date=date(2000, 1, 1))
        t = normalize_ticket(row, None, extracted, now=NOW)
    ...
    def test_pdf_path_stored(self):
        row = _make_row()
        t = normalize_ticket(row, None, None, pdf_path="/data/raw/pdf/TN20240601-100001.pdf", now=NOW)
        assert t.source_pdf_path == "/data/raw/pdf/TN20240601-100001.pdf"

    def test_pdf_sha256_stored(self):
        row = _make_row()
        t = normalize_ticket(row, None, None, pdf_sha256="abc123", now=NOW)
        assert t.source_pdf_sha256 == "abc123"
    ...
    def test_whitespace_collapsed_in_fields(self):
        row = _make_row()
        extracted = _make_extracted(excavator_name="  Acme   Fiber   LLC  ")
        t = normalize_ticket(row, None, extracted, now=NOW)
    ...
    def test_content_hash_stable(self):
        ...
        t1 = normalize_ticket(row, None, extracted, now=NOW)
        t2 = normalize_ticket(row, None, extracted, now=NOW)
        assert t1.content_hash() == t2.content_hash()

    def test_content_hash_changes_on_different_data(self):
        row = _make_row()
        e1 = _make_extracted(excavator_name="Company A")
        e2 = _make_extracted(excavator_name="Company B")
        t1 = normalize_ticket(row, None, e1, now=NOW)
        t2 = normalize_ticket(row, None, e2, now=NOW)
        assert t1.content_hash() != t2.content_hash()
