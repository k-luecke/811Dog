...
def test_target_packages_split_into_three_subfolders(tmp_path, sample_views):
    out = tmp_path / "out.kmz"
    _build_and_save(
        views_relevant=sample_views,
        views_contractor_other=[],
        ...
        now_ct=NOW_CT,
        nashdigs_target=[
            _fake_target_pkg(name="BNA100a", status="Started", project_id=1),
            _fake_target_pkg(name="BNA100b", status="Committed", project_id=2,
                             act_start=date(2026, 5, 1), act_end=date(2026, 9, 1)),
            # Recently-completed (ActEnd within 30 days of NOW_CT 2026-04-24)
            _fake_target_pkg(name="BNA100c", status="Started", project_id=3,
...
def test_conflict_folder_only_appears_when_non_target_packages_present(tmp_path, sample_views):
    """Without non-Northstar packages, the conflict folder must not be created."""
    out = tmp_path / "out.kmz"
    _build_and_save(
        views_relevant=sample_views,
        views_contractor_other=[],
        out_path=out,
        now_ct=NOW_CT,
        nashdigs_target=[_fake_target_pkg()],
        nashdigs_others=[],
    )
    root = _parse_kmz(out)
    doc = root.find("kml:Document", _NS)
...
def test_package_popup_includes_ticket_counts(tmp_path, sample_views):
    ...
    _build_and_save(
        ...
        out_path=out,
        now_ct=NOW_CT,
        nashdigs_target=[_fake_target_pkg(name="BNA999a", project_id=42)],
