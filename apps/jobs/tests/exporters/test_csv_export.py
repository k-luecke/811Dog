...
def test_by_sub_csv(tmp_path, sample_views):
    ...
    delta = by_name["DELTA CABLE"]
    assert delta["active_count"] == "1"  # T2
    assert delta["cancelled_last_7d_count"] == "1"  # T5
    assert delta["late_utilities_count"] == "1"  # T2
...
class TestSlugify:
    ...
    def test_numbers_preserved(self):
        assert slugify("DTX Brothers 2") == "dtx-brothers-2"
